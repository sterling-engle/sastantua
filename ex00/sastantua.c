...
/*   Updated: 2019/07/14 23:09:32 by sengle           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
** PM_ROWS(size) macro returns the number of rows for pyramid size
** DOOR_WIDTH(size) macro returns the width of the door for pyramid size
*/

#define PM_ROWS(size)	((2 * size) + ((size * (size + 1)) / 2))
...
**					C. else output '|' door chars for entire DOOR_WIDTH(size)
**					D. output the rest of the '*' after the door
**				4. else output stars * 2 + 1 '*' characters across pyramid row
...
static void	draw_row(int size, int row, int spaces, int stars)
{
	putnchars(spaces, ' ');
	ft_putchar('/');
	if (row >= PM_ROWS(size) - DOOR_WIDTH(size))
	{
		putnchars(stars - (DOOR_WIDTH(size) - 1) / 2, '*');
		if (size >= 5 && row == PM_ROWS(size) - DOOR_WIDTH(size) / 2 - 1)
		{
...
**				stars[]:	 array of integers containing no. of stars per row
**
**				Algorithm:
**
**				1. while the pyramid level is less than or equal to its size:
**					A. for each line in the level,
**					   calculate the stars_count for that row
**				2. while the row stars[] index is less than the pyramid's rows:
**					A. draw the row using
**					   draw_row(int size, int row, int spaces, int stars)
*/
...
void		sastantua(int size)
{
	...
	row = -1;
	while (++row < PM_ROWS(size))
		draw_row(size, row, stars[PM_ROWS(size) - 1] - stars[row], stars[row]);
}
